...
export const GlobalProtocolMetricsSkeleton = () => {
  return (
    <View direction="row" gap={6} wrap className="justify-between">
      <Card className="flex-1 min-w-[200px]">
        <View padding={6} gap={2}>
          <Text variant="body-2" color="neutral-faded">24h Volume</Text>
            ...
            <View direction="row" align="center" gap={1}>
              {displayChange > 0 ? (
                <>
                  ...
                  <Text variant="body-3" color="positive">{Math.abs(displayChange).toFixed(2)}%</Text>
                </>
              ) : displayChange < 0 ? (
                <>
                  <ArrowDown size={16} color="var(--rs-color-critical)" weight="bold" />
                  <Text variant="body-3" color="critical">{Math.abs(displayChange).toFixed(2)}%</Text>
                </>
              ) : (
